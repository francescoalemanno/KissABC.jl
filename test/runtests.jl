...
res,_=ABCSMCPR(prior,sim,tdata,ksdist,0.1,nparticles=1000,parallel=true,maxsimpp=1000,c=0.001)

prsample=[rand(prior) for i in 1:1000]
...
subplot(2,2,1)
title("PRIOR")
hist(μ_pr,histtype="step",label=L"π(μ)")
xlim(dilateextrema(μ_pr)...)
legend()
xlabel(L"\mu")
subplot(2,2,2)
title("POSTERIOR")
hist(μ_p,histtype="step",label=L"P(μ|{\rm data})")
xlim(dilateextrema(μ_pr)...)
legend()
xlabel(L"\mu")
subplot(2,2,3)
hist(σ_pr,histtype="step",label=L"π(σ)")
xlim(dilateextrema(σ_pr)...)
xlabel(L"\sigma")
legend()
subplot(2,2,4)
hist(σ_p,histtype="step",label=L"P(σ|{\rm data})")
xlim(dilateextrema(σ_pr)...)
xlabel(L"\sigma")
legend()
tight_layout()
